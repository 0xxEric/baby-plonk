...
class Prover:
    ...
    def round_3(self) -> Message3:
        ...
        permutation_grand_product_coeff = (
            # TODO: your code
            (
                self.rlc(A_coeff, roots_coeff)* self.rlc(B_coeff, k1*roots_coeff) * self.rlc(C_coeff, k2*roots_coeff)
            )
            * Z_coeff
            - (self.rlc(A_coeff, S1_coeff)* self.rlc(B_coeff, S2_coeff)* self.rlc(C_coeff, S3_coeff)
            )
            * ZW_coeff
        )
